...
test("ฝั่งแอปเขียนเวลาลงช่อง duration_blocks เดิม ไม่เพิ่มคอลัมน์ใหม่ในคิวตอนเปิดใช้ครั้งแรก", () => {
  // เฟสแรก (นี้) ตั้งใจไม่เก็บว่าเลือกบริเวณไหน เอาแค่ผลรวมเวลาไปเขียนทับ duration_blocks
  // เพื่อไม่ต้องแตะตาราง queues เลยตอนเปิดใช้ครั้งแรก (ความเสี่ยงต่ำสุด)
  //
  // *** เฟสถัดไป (16 ก.ย. 2569) เปลี่ยนแล้ว *** — เจ้าของงานเจอเองว่าบันทึกคิวแล้ว
  // ไม่มีที่ไหนบอกว่าเลือกบริเวณไหน มีแต่ตัวเลขเวลา จึงเพิ่มคอลัมน์ queues.area_names
  // (migration 20260916100000) เป็นข้อความ snapshot ไว้แสดงผลเท่านั้น — ไม่ใช่ตัวตัดสิน
  // เวลา duration_blocks ยังทำหน้าที่นั้นเหมือนเดิมทุกอย่าง ดู test/areaNamesDisplay.test.js
  assert.ok(/areaNames: q\.area_names \|\| ""/.test(service),
    "mapQueueRow ต้องแปลง area_names — ถ้าไม่มีแปลว่าคอลัมน์ area_names หายไปจากฝั่งแอป");
});
