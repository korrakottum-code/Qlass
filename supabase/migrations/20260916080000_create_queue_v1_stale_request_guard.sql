-- ปิดรูสุดท้ายของ "รหัสอ้างอิงเดิม + ข้อมูลเปลี่ยน" และแยกข้อความเตือนออกมาให้ชัด
--
-- ═══ ปัญหา 1: ทางสำรองไม่มีด่าน ═══
-- 20260913090000 เติมด่านเทียบข้อมูลไว้ที่ทางหลัก (select ... where request_id) แล้ว
-- แต่ปลายฟังก์ชันยังมีทางสำรองอีกเส้นที่ไม่มีด่าน:
--
--     exception when unique_violation then
--       select * into v_existing ...;
--       if found and v_existing.recorded_by = p_actor_staff_id then
--         return to_jsonb(v_existing);   -- คืนใบเดิมโดยไม่เทียบข้อมูลเลย
--
-- เส้นนี้ทำงานเมื่อสองคำขอที่ถือรหัสอ้างอิงเดียวกันวิ่งชนกันจนหลุดล็อก แล้ว unique index
-- เป็นคนจับแทน โอกาสเกิดน้อย แต่ผลเหมือนกันเป๊ะกับรูที่ปิดไปแล้ว คือหน้าจอขึ้นว่าบันทึก
-- สำเร็จทั้งที่สิ่งที่แก้ไม่ได้ลง คิวจึงอยู่ผิดเวลา/ผิดห้องโดยไม่มีใครรู้ตัว
--
-- ═══ ปัญหา 2: ข้อความเตือนบอกผิดทาง ═══
-- ของเดิมใช้รหัส invalid_queue_payload ร่วมกับอีกหลายกรณี หน้าจอจึงขึ้นว่า
-- "ข้อมูลไม่ผ่านการตรวจสอบจากเซิร์ฟเวอร์ กรุณาตรวจสอบฟอร์ม" ซึ่งชวนให้แอดมินไปแก้ฟอร์ม
-- แล้วกดบันทึกใหม่ — แต่ความจริงคือคิวลงไปแล้วตั้งแต่ครั้งก่อน กดใหม่จะได้สองคิว
-- แยกเป็นรหัสของตัวเอง request_id_stale เพื่อให้หน้าจอบอกได้ตรงว่า "ไปแก้ที่คิวเดิม"
--
-- รหัสใหม่ถูกเพิ่มเข้า allowlist ของ staff-session และตารางข้อความของ queueCreateGate.js
-- ไปก่อนแล้วใน PR เดียวกัน และ Edge Function ถูก deploy ก่อนไฟล์นี้ — ถ้าสลับลำดับ
-- รหัสที่สองฝั่งไม่รู้จักจะถูกตีเป็นเน็ตล่ม แล้วรหัสอ้างอิงเดิมจะถูกเก็บไว้ กดใหม่ก็วนเจอเดิมไม่จบ
--
-- ═══ เปลี่ยนอะไร ═══
-- 1. ฟังก์ชันช่วย queue_payload_unchanged — ยกเงื่อนไขเทียบ 12 ช่องออกมาไว้ที่เดียว
--    ทั้งสองทางเรียกตัวเดียวกัน กันไม่ให้แก้ที่เดียวแล้วอีกที่ค้างของเก่า (บทเรียนจาก
--    20260913100000 ที่ฝั่งแอปกับฝั่งเซิร์ฟเวอร์ค้างกติกาคนละชุดอยู่เดือนกว่า)
-- 2. ทางหลัก: เรียกฟังก์ชันช่วย และเปลี่ยนรหัสเป็น request_id_stale
-- 3. ทางสำรอง: เติมด่านเดียวกันเข้าไป
-- ที่เหลือคัดลอกมาทั้งดุ้นจาก 20260913100000 — พิสูจน์ด้วยการแทนที่ย้อนกลับแล้ว md5
-- ของส่วนที่เหลือเท่าของเดิมเป๊ะ (c6f6ca0b542ea78c5ee7d57bc02a92df)
--
-- เงื่อนไขเทียบข้อมูลเหมือนเดิมทุกช่อง ไม่ได้เข้มขึ้นหรือผ่อนลง — คิวที่กดซ้ำเพราะเน็ตกระตุก
-- (ข้อมูลเหมือนเดิมเป๊ะ) ยังได้ใบเดิมกลับเหมือนเดิม ไม่เกิดคิวซ้ำ

-- หน้าร้านเปิดอยู่: ถ้าจับล็อกไม่ได้ใน 3 วินาที ให้ยกเลิกทั้งก้อนไปเลย ดีกว่าค้างขวางคิวคนอื่น
...
comment on function public.queue_payload_unchanged(public.queues, jsonb) is
  'True when an existing queue row still matches the payload that carried its request_id. Shared by both idempotency paths of create_queue_v1 so the two can never drift apart.';
