-- create_queue_v1: ไม่เดา "ประเภทลูกค้า" ให้เองอีกต่อไป
--
-- ═══ ทำไม ═══
-- ประเภทลูกค้าไปผูกกับเรตค่าคอมมิชชั่น และตัวเลข "ต้นทุนต่อลูกค้าใหม่" ในหน้า CEO
-- (จำนวนลูกค้าใหม่ หารกับค่าโฆษณา) แต่ทุกทางที่ลงคิวเคยเลือกให้ล่วงหน้า แล้วค่าที่
-- ระบบเลือกให้ก็ถูกกดผ่าน — ข้อมูล 1 มิ.ย. - 8 ก.ย. 2026: คิวที่ไม่ใช่ครั้งแรกของ
-- เบอร์นั้นมี 36,583 คิว ในจำนวนนี้ 8,065 คิว (22%) ยังถูกติ๊กว่า "ลูกค้าใหม่"
--
-- ฝั่งหน้าจอแก้แล้ว (ฟอร์มหุบครึ่งล่างไว้จนกว่าจะเลือกเอง) แต่ถ้าเซิร์ฟเวอร์ยังเติม
-- 'new' ให้เมื่อไม่ได้ส่งค่ามา รูรั่วก็ยังอยู่สำหรับทุกคนที่ยิงเข้า RPC นี้
--
-- ═══ เปลี่ยนอะไร (แค่ 2 บรรทัดในฟังก์ชันเดิม ที่เหลือคัดลอกมาทั้งดุ้น) ═══
--   1. v_customer_type ไม่ coalesce เป็น 'new' อีกต่อไป — ไม่ส่งมา = null
--   2. เพิ่ม "is null" ในเงื่อนไขตรวจ เพราะ NULL not in (...) ให้ผลเป็น NULL ไม่ใช่ true
--      ถ้าไม่เพิ่ม if จะไม่ทำงาน แล้วคิวที่ไม่มีประเภทจะหลุดลงตารางเงียบ ๆ
--      (ตาราง queues.customer_type เป็น nullable ไม่มี default จึงไม่มีอะไรกันไว้อีกชั้น)
--
-- ไม่ส่งประเภทมา -> 'invalid_queue_payload' ซึ่งเป็น error code เดิมที่ Edge Function
-- กับฝั่งแอปรู้จักอยู่แล้ว ไม่ต้องแก้ทั้งสองที่
--
-- ═══ ปลอดภัยกับคนที่ใช้อยู่ ═══
-- ฝั่งแอปเวอร์ชันใหม่บังคับเลือกประเภทอยู่แล้ว จึงส่งค่ามาเสมอ
-- เครื่องที่ยังรันเวอร์ชันเก่า (ยังไม่รีเฟรช) ส่ง 'new' มาตามค่าเดิมของฟอร์ม ผ่านปกติ
-- ทางนี้รับเฉพาะคิวที่สร้างใหม่สถานะ pending/confirmed ของแอดมินใน allowlist เท่านั้น
-- คิวรอและการแก้คิวเดิมไม่ได้ผ่านทางนี้
--
-- Rollback: รัน 20260817160000_create_queue_v1_room_procedure_lock.sql ซ้ำ
--           (ฟังก์ชันเวอร์ชันก่อนหน้าอยู่ในไฟล์นั้นทั้งดุ้น)
