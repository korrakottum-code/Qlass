-- Goal 13: server-only, idempotent queue creation foundation.
--
-- The function is intentionally not granted to browser roles.  An Edge
-- Function validates the staff session, then calls this RPC with the trusted
-- staff and session IDs.  Existing browser writes remain unchanged until a
-- later, separately reviewed client cutover.
...
begin
  ...
  if v_customer_type not in ('new', 'old', 'course') or v_status <> 'pending' or coalesce(v_price, 0) < 0 then
    raise exception using errcode = 'P0001', message = 'invalid_queue_payload';
  end if;
  ...
  insert into public.queues (
    ...
    v_date, v_time_block, v_duration, v_room_id, 'pending', '', p_actor_staff_id,
...
comment on function public.create_queue_v1(uuid, uuid, uuid, jsonb) is
  'Goal 13: server-only idempotent queue creation. Browser roles have no execute grant.';
