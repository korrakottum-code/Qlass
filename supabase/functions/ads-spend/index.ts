...
// ถ้าปลายทางค้าง ต้องไม่ปล่อยให้ function ค้างยาว แต่ต้องเผื่อ cold start ของ Vercel
// + เวลาที่ปลายทางไปดึงจาก Meta (วัดจริงเกิน 10 วิได้) — 25 วิ ยังต่ำกว่าเพดานของ Supabase มาก
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      console.error("ads-spend upstream unreachable after ms", Date.now() - startedAt, fetchError);
      return response({ error: "upstream_timeout" }, 504, origin);
    }
    if (!upstream.ok) {
      // ไม่ส่งรายละเอียด/สถานะภายในของปลายทางกลับไปฝั่ง client มากกว่าที่จำเป็น
      console.error("ads-spend upstream error", upstream.status, "after ms", Date.now() - startedAt);
      if (upstream.status === 401) {
        // ลายนิ้วมือของ token ที่อ่านได้จริง — ไม่เปิดเผยค่า แต่ชี้ขาดได้ว่าคนละคีย์หรือมี whitespace ติดมา
        console.error("ads-spend token fingerprint", JSON.stringify({
          len: adsToken.length,
          trimmedLen: adsToken.trim().length,
          sha16: (await sha256(adsToken)).slice(0, 16),
          upstreamBody: (await upstream.clone().text()).slice(0, 200),
        }));
      }
    ...
    const daily = Array.isArray(json?.daily)
      ...
      : null;
    console.log("ads-spend ok", JSON.stringify({ since, until, ms: Date.now() - startedAt, spend: Number(json?.spend) || 0, dailyRows: daily?.length ?? null }));
