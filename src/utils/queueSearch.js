...
// ตัวคั่นของ PostgREST .or() คือ comma กับวงเล็บ, * กับ % _ เป็น wildcard ของ ilike,
// ส่วน " ใช้ครอบค่าในไวยากรณ์ตัวกรอง — ถ้าหลุดเข้าไปในคำค้นจะทำให้ค้นมั่ว
// (พิมพ์ ก*ข แล้วได้ "สมปรารถนา ปทักขินัง" เพราะ * กลายเป็นไวลด์การ์ด) หรือได้ผลเปล่า
// ถอดทิ้งที่เดียวตรงนี้ ทั้งฝั่งฐานข้อมูลและฝั่งจับคู่ในหน้าจอจะได้ตีความคำค้นตรงกัน
//
// จุด (.) ปล่อยไว้ได้ PostgREST ตัดแค่สองจุดแรกเป็น column.operator ที่เหลือเป็นค่า
// ชื่อที่ขึ้นต้นด้วย "น.ส." จึงยังค้นเจอปกติ (ตรวจกับฐานข้อมูลจริงแล้ว)
const UNSAFE = /[,()%_*"\\]/g;
