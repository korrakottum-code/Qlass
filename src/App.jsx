...
export default function App() {
  ...
  const handleBookingSubmit = useCallback(async () => {
    ...
    // เลือกก่อนบันทึกเสมอ — เจ้าของงานยืนยันว่าธุรกิจนี้ทุกคิวผูกโปรจริงอยู่แล้ว
    // ไม่มีแนวคิด "ราคาปกติไม่มีส่วนลด" จึงไม่มีโปรสำรองให้เลือก ต้องเป็นโปรจริงเท่านั้น
    //
    // ผลข้างเคียงที่รู้ตัวและเจ้าของงานรับแล้ว (16 ก.ย. 2569): 2 หัตถการที่ยังไม่มีโปร
    // ผูกไว้เลยสักตัว (Go-Filler, Ulthera — ทั้งคู่มีคิวจองจริงอยู่) จะบันทึกคิวใหม่
    // ไม่ได้จนกว่าจะมีการเพิ่มโปรให้หัตถการนั้นในหน้า "จัดการโปร/แพ็กเกจ" — ไม่ใช่บั๊ก
    //
    // ขอบเขต: เฉพาะคิวใหม่ที่เลือกหัตถการแล้ว (ยังไม่บังคับกับคิวรอ — ตั้งใจให้ลงคิวรอ
    // ได้ไวเหมือนเดิม ยังไม่รู้รายละเอียดตอนแรกจับจอง) และไม่บังคับย้อนหลังกับคิวเก่า
    // ที่กำลังแก้ไข (เจ้าของงานยืนยันแล้วว่าเอาแค่จากนี้ไปพอ)
    if (!editingQueueId && form.status !== "waiting_queue" && form.procedureId && !form.promoId) {
      showToast("error", "กรุณาเลือกโปร/แพ็กเกจก่อนบันทึกคิว");
      return;
    }
            ...
            {page === "booking" && (
              <BookingPage
                ...
                onBulkBooking={async (allFields) => {
                  ...
                  //
                  // ตั้งใจไม่บังคับเลือกโปรในทางนี้ (ต่างจาก handleBookingSubmit/Timeline)
                  // เพราะเป็นการแยกข้อความอิสระ ไม่มีฟอร์มให้เลือก — ตัวแยกอ่านชื่อโปรจาก
                  // ข้อความได้ไม่แม่นเท่าคำสั้น ๆ อย่าง "ลูกค้าใหม่/เก่า/คอร์ส" ถ้าบังคับ
                  // ด้วยจะทำให้แทบทุกแถวถูกข้ามจนฟีเจอร์นี้ใช้ไม่ได้จริง
            ...
            {page === "timeline" && (
              <TimelinePage
                ...
                onSubmitBooking={async (bookingForm) => {
                  ...
                  // (ปุ่มฝั่ง UI ก็ disabled ไว้แล้ว นี่คือด่านสำรอง) Timeline สร้างคิวใหม่
                  // เสมอไม่มีคิวรอ จึงไม่ต้องเช็ค status แยกแบบหน้าบันทึกคิว
                  if (bookingForm.procedureId && !bookingForm.promoId) {
                    showToast("error", "กรุณาเลือกโปร/แพ็กเกจก่อนบันทึกคิว");
                    return false;
                  }
